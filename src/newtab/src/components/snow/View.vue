...
export default class SnowView extends Vue {
  ...
  get snowflakes (): Snowflake[] {
    return [...Array(this.config.amount)].map((_, i): Snowflake => {
      ...
      return {
        ...
        outer: {
          ...
          animationDuration: `${Math.random() * this.fallDuration.variable + this.fallDuration.base}s`,
...
@keyframes snowflake-horizontal {
  from, to { transform: translateX(-200%); }
